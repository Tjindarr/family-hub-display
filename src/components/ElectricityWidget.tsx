...
function getPriceColor(price: number): string {
  if (price < 0.5) return "hsl(120, 50%, 50%)";
  if (price < 1.0) return "hsl(32, 95%, 55%)";
  return "hsl(0, 72%, 55%)";
}

function getPriceBadgeClass(price: number): string {
  if (price < 0.5) return "price-badge-low";
  if (price < 1.0) return "price-badge-mid";
...
export default function ElectricityWidget({ nordpool, loading, fontSizes }: ElectricityWidgetProps) {
  ...
  const minPoint =
    futurePoints.length > 0 ? futurePoints.reduce((min, p) => (p.price < min.price ? p : min), futurePoints[0]) : null;
  ...
  const ticks = chartData.filter((_, i) => i % 3 === 0).map((d) => d.time);
  ...
  return (
    <div className="widget-card h-full flex flex-col overflow-hidden">
      ...
      {loading ? (
        ...
        <div className="flex-1 min-h-0 px-[2px] py-[5px]" style={{ marginTop: "-15px", marginBottom: "-15px" }}>
      ...
      <div className="mt-0 flex gap-6 shrink-0" style={{ fontSize: fs.label }}>
        ...
        <div className="ml-auto flex gap-4">
          ...
          <div>
            ...
            {minTimeStr && <span className="text-muted-foreground ml-1">@ {minTimeStr}</span>}
          </div>
          <div>
