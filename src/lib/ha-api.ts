...
class HomeAssistantAPI {
  ...
  ): Promise<Record<string, { start: number; end: number; mean?: number; min?: number; max?: number; sum?: number; change?: number; state?: number }[]>> {
    // Use WebSocket API since REST doesn't support statistics
    const wsUrl = this.baseUrl.replace(/^http/, "ws") + "/api/websocket";

    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        ws.close();
        reject(new Error("WebSocket statistics request timed out"));
      }, 15000);
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (msg.type === "auth_required") {
            ...
            if (endTime) req.end_time = endTime;
            ...
            if (msg.success) {
              ...
              reject(new Error("Statistics request failed: " + JSON.stringify(msg.error)));
            }
          }
        } catch (e) {
          clearTimeout(timeout);
